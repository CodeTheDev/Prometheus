...
public class Disconnect extends ListenerAdapter {
	...
	public static final String NAME = "p!disconnect | p!leave";
	public static final String DESCRIPTION = "Disconnects the bot from any voice channel.";
	...
	public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
		...
		if (msgContent.equalsIgnoreCase("p!disconnect") || msgContent.equalsIgnoreCase("p!leave")) {
			...
			if ((!executor.hasPermission(Permission.ADMINISTRATOR) && executor.getRoles().contains(dj)) || executor.hasPermission(Permission.ADMINISTRATOR) || executor.getRoles().contains(dj)) {
				...
				if (!VC.getMembers().contains(executor)) {
					...
					channel.sendMessageFormat(":x: **|** Do not try and remotely disconnect me from **%s** %s", VC.getName(), executor.getAsMention()).queue();
					return;

				}

				manager.getGuildMusicManager(guild).player.destroy(); // Destroy PlayerManager instance.
				audio.closeAudioConnection(); // Close audio connection.
