...
public class Stop extends ListenerAdapter {
	...
	public static final String NAME = "p!stop";
	public static final String DESCRIPTION = "Stops the current track the bot is playing. Unlike pause, this will remove the track entirely.";
	...
	public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
		...
		if (msgContent.equalsIgnoreCase("p!stop")) {
			...
			if ((!executor.hasPermission(Permission.ADMINISTRATOR) && executor.getRoles().contains(dj)) || executor.hasPermission(Permission.ADMINISTRATOR) || executor.getRoles().contains(dj)) {
				...
				if (!VC.getMembers().contains(executor)) {
					...
					channel.sendMessageFormat(":x: **|** Do not try and remotely stop me %s", executor.getAsMention()).queue();
					return;

				}
				
				manager.getGuildMusicManager(guild).player.stopTrack(); // Stop current track.
				channel.sendMessage(":stop_button: **|** *Stopped!*").queue();
