...
public class Pause extends ListenerAdapter {
	...
	public static final String NAME = "p!pause";
	public static final String DESCRIPTION = "Pauses the current track the bot is playing.";
	...
	public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
		...
		if (msgContent.equalsIgnoreCase("p!pause")) {
			...
			if ((!executor.hasPermission(Permission.ADMINISTRATOR) && executor.getRoles().contains(dj)) || executor.hasPermission(Permission.ADMINISTRATOR) || executor.getRoles().contains(dj)) {
				...
				if (!VC.getMembers().contains(executor)) {
					...
					channel.sendMessageFormat(":x: **|** Do not try and remotely pause me %s", executor.getAsMention()).queue();
					return;

				}
				
				manager.getGuildMusicManager(guild).player.setPaused(true); // Pause player.
